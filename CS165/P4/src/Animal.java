...
public abstract class Animal {
    ...
    void findClosest(){
		...
		for (int i = 0; i < field.length; i++) {
			for (int j = 0; j < field[0].length; j++) {
				if(field[i][j] == lookFor) {
					if(minimum > computeDistance(currentRow, currentCol, i, j)) {
							minimum = computeDistance(currentRow, currentCol, i, j);
							closestRow = i;
							closestCol = j;
					}
				}
	...
	double computeDistance(int row0, int col0, int row1, int col1) {
		...
		double x = 0.0;
		double y = 0.0;
		x = Math.pow(Math.abs(row1 - row0), 2);
		y = Math.pow(Math.abs(col1 - col0), 2);
